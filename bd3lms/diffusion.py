...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    if self.config.algo.backbone == 'dit':
      ...
      ckpt_or_model_id = self.config.eval.checkpoint_path
      if (isinstance(ckpt_or_model_id, str)
          and ckpt_or_model_id.endswith('.ckpt')):
        raise ValueError(
          "algo.backbone=hf_dit expects a HuggingFace model directory or Hub repo_id, "
          f"but got a Lightning checkpoint file: {ckpt_or_model_id}. "
          "If you're loading a local `.ckpt`, set `algo.backbone=dit` (recommended). "
          "Alternatively, convert/export the checkpoint to a HF model (see `push_to_hf.py`)."
        )
      self.backbone = transformers.AutoModelForMaskedLM.from_pretrained(
        ckpt_or_model_id, trust_remote_code=True)
  ...
  def _reset_backbone_kv_cache(self):
    """Reset KV cache for sampling.

    Different backbones expose different `reset_kv_cache` signatures:
    - Native DiT (`models/dit.py`): reset_kv_cache()
    - HF BD3LM (`models/hf/modeling_bd3lm.py`): reset_kv_cache(eval_batch_size=...)
    """
    if not hasattr(self.backbone, 'reset_kv_cache'):
      return

    bs = getattr(self.config.loader, 'eval_batch_size', 1)
    # Prefer keyword form (HF model), then fall back to positional / no-arg.
    try:
      self.backbone.reset_kv_cache(eval_batch_size=bs)
      return
    except TypeError:
      pass
    try:
      self.backbone.reset_kv_cache(bs)
      return
    except TypeError:
      pass
  ...
  def restore_model_and_sample(self, num_steps, eps=1e-5, seqlen=None):
    ...
    samples = self._sample(
      ...
      num_steps=num_steps,
      eps=eps)
    if getattr(self.config.eval, 'compute_gen_metrics', True):
