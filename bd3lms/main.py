...
def generate_samples(config, logger, tokenizer):
  ...
  if getattr(config.eval, 'compute_gen_metrics', True):
  ...
  if getattr(config.eval, 'compute_gen_metrics', True):
    gen_ppls = model.metrics.gen_ppls
    gen_entropies = model.metrics.gen_entropies
    gen_lengths = model.metrics.gen_lengths
  else:
    # Keep output stable without forcing any eval-model downloads.
    gen_ppls = [None for _ in range(num_samples)]
    gen_entropies = [None for _ in range(num_samples)]
    gen_lengths = [None for _ in range(num_samples)]

  save_dict = {
    'gen_ppl': gen_ppls,
    'gen_nfes': model.metrics.gen_nfes,
    'gen_entropy': gen_entropies,
    'gen_lengths': gen_lengths,
    'samples': [[i] for i in text_samples],
    'seed': [config.seed for _ in range(num_samples)],
  }
